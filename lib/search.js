...
export async function searchDocuments(queryText, optionsOrDocIds = {}, legacyTopK = undefined) {
  ...
  // If metadata filters are specified, or activeOnly with no specific docs, use filtered query
  const needsFiltered = statusFilter || excludeStatuses || metadataFilters || legalHoldOnly || (activeOnly && documentIds.length === 0);
  if (needsFiltered) {
 ...
 */
export function scoreDocumentsByTags(queryTags, topN = 15, activeOnly = false) {
