 ...
 * @param {number} [options.orgId] - Organization ID filter
 *
...
export async function searchDocuments(queryText, optionsOrDocIds = {}, legacyTopK = undefined) {
  ...
  // If metadata filters are specified, or activeOnly with no specific docs, or orgId is specified, use filtered query
  const needsFiltered = statusFilter || excludeStatuses || metadataFilters || legalHoldOnly || (activeOnly && documentIds.length === 0) || orgId !== undefined;
  if (needsFiltered) {
...
function getChunksFiltered(filters) {
  ...
  // Active documents only — exclude superseded and archived
  if (filters.activeOnly) {
    conditions.push("d.superseded_by IS NULL");
    conditions.push("(d.status != 'archived' OR d.status IS NULL)");
  }
...
/**
 * Format search results for Claude context — document names as headers, no numbering.
 * Claude is instructed to reference document names naturally in its answer.
 * @param {Array} results - Search results from searchDocuments
 * @param {Map<number, number>} chunkCounts - unused, kept for backward compat
 * @returns {string} - Formatted string for Claude prompt
 */
 ...
 * @param {boolean} [activeOnly=false] - Only include active documents
 * @param {number} [orgId] - Organization ID filter
 * @returns {number[]} - Array of document IDs sorted by tag relevance
 */
export function scoreDocumentsByTags(queryTags, topN = 15, activeOnly = false, orgId = undefined) {
  ...
  if (orgId !== undefined) {
    sql = activeOnly
      ? `SELECT id, tags FROM documents WHERE processed = 1 AND tags IS NOT NULL AND superseded_by IS NULL AND (status != 'archived' OR status IS NULL) AND org_id = ?`
      : `SELECT id, tags FROM documents WHERE processed = 1 AND tags IS NOT NULL AND org_id = ?`;
    params.push(orgId);
  } else {
    sql = activeOnly
      ? `SELECT id, tags FROM documents WHERE processed = 1 AND tags IS NOT NULL AND superseded_by IS NULL AND (status != 'archived' OR status IS NULL)`
      : `SELECT id, tags FROM documents WHERE processed = 1 AND tags IS NOT NULL`;
  }

  const docs = dbQuery(sql, params);
