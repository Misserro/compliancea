 ...
 * @returns {Object[]}
 */
export function getAllPolicies(enabledOnly = false, orgId = undefined) {
  if (orgId !== undefined) {
    const where = enabledOnly ? "WHERE enabled = 1 AND org_id = ?" : "WHERE org_id = ?";
    return query(
 ...
 * @param {Object} [auditOptions] - Audit options { userId, orgId }
 * @returns {number} - New policy ID
 */
export function createPolicy(name, condition, actionType, actionParams = {}, orgId = null, auditOptions = {}) {
  ...
  const result = run(
    `INSERT INTO policy_rules (name, condition_json, action_type, action_params, org_id)
     VALUES (?, ?, ?, ?, ?)`,
    [
      name,
 ...
 * @returns {boolean}
 */
export function updatePolicy(id, updates, auditOptions = {}) {
  const fields = [];
  const params = [];
  ...
  logAction("policy", id, "updated", updates, auditOptions);
 ...
 * @returns {boolean}
 */
export function deletePolicy(id, auditOptions = {}) {
  const policy = getPolicyById(id);
  const result = run("DELETE FROM policy_rules WHERE id = ?", [id]);
  if (result.changes > 0) {
    logAction("policy", id, "deleted", { name: policy?.name }, auditOptions);
  }
  return result.changes > 0;
}
 ...
 * @param {number} [orgId] - Organization ID to scope policies
 * @returns {{policy: Object, action: string, params: Object}[]}
 */
export function evaluateDocument(document, orgId = undefined) {
  const policies = getAllPolicies(true, orgId); // enabled only
  const triggered = [];
 ...
 */
export function applyActions(documentId, triggeredActions, auditOptions = {}) {
  const applied = [];
  let tasksCreated = 0;
  ...
  for (const { policy, action, params } of triggeredActions) {
    switch (action) {
      ...
      case "require_approval": {
        run(
          `INSERT INTO tasks (title, description, entity_type, entity_id, task_type, org_id)
 ...
 * @param {number} [orgId] - Organization ID to scope documents
 * @returns {{matches: Object[], totalDocuments: number}}
 */
export function testPolicy(policyId, orgId = undefined) {
