...
export function insertObligation({ documentId, obligationType, title, description, clauseReference, dueDate, recurrence, noticePeriodDays, owner, escalationTo, proofDescription, evidenceJson, category, activation, summary, detailsJson, penalties, stage }) {
  const result = run(
    `INSERT INTO contract_obligations (document_id, obligation_type, title, description, clause_reference, due_date, recurrence, notice_period_days, owner, escalation_to, proof_description, evidence_json, category, activation, summary, details_json, penalties, stage)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [documentId, obligationType, title, description || null, clauseReference || null, dueDate || null, recurrence || null, noticePeriodDays || null, owner || null, escalationTo || null, proofDescription || null, evidenceJson || "[]", category || null, activation || "active", summary || null, detailsJson || "{}", penalties || null, stage || "active"]
  );
  return result.lastInsertRowId;
}
...
export function updateObligation(id, updates) {
  const allowedFields = ["obligation_type", "title", "description", "clause_reference", "due_date", "recurrence", "notice_period_days", "owner", "escalation_to", "proof_description", "evidence_json", "status", "category", "activation", "summary", "details_json", "penalties", "stage"];
  const fields = [];
  const params = [];
...
export function transitionObligationsByStage(documentId, newStage, previousStage) {
  // Activate obligations for the new stage
  run(
    `UPDATE contract_obligations SET status = 'active', updated_at = CURRENT_TIMESTAMP
     WHERE document_id = ? AND stage = ? AND status NOT IN ('met', 'waived')`,
    [documentId, newStage]
  );

  // Finalize obligations from the previous stage (except payments which carry over)
  if (previousStage && previousStage !== newStage) {
    if (newStage === "terminated") {
      // When terminating: finalize previous stage obligations EXCEPT payments
      run(
        `UPDATE contract_obligations SET status = 'finalized', updated_at = CURRENT_TIMESTAMP
         WHERE document_id = ? AND stage = ? AND status = 'active' AND category != 'payment'`,
        [documentId, previousStage]
      );
    } else {
      run(
        `UPDATE contract_obligations SET status = 'finalized', updated_at = CURRENT_TIMESTAMP
