 ...
 */
export function logTokenUsage({ userId, orgId, route, model, inputTokens, outputTokens, voyageTokens = 0, costUsd = 0 }) {
  try {
    run(
      `INSERT INTO token_usage (user_id, org_id, route, model, input_tokens, output_tokens, voyage_tokens, cost_usd)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [userId, orgId, route, model, inputTokens, outputTokens, voyageTokens, costUsd]
    );
  } catch (e) {
