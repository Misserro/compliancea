...
export function insertObligation({ documentId, obligationType, title, description, clauseReference, dueDate, recurrence, noticePeriodDays, owner, escalationTo, proofDescription, evidenceJson, category, activation, summary, detailsJson, penalties }) {
  const result = run(
    `INSERT INTO contract_obligations (document_id, obligation_type, title, description, clause_reference, due_date, recurrence, notice_period_days, owner, escalation_to, proof_description, evidence_json, category, activation, summary, details_json, penalties)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [documentId, obligationType, title, description || null, clauseReference || null, dueDate || null, recurrence || null, noticePeriodDays || null, owner || null, escalationTo || null, proofDescription || null, evidenceJson || "[]", category || null, activation || "active", summary || null, detailsJson || "{}", penalties || null]
  );
  return result.lastInsertRowId;
}
...
export function updateObligation(id, updates) {
  const allowedFields = ["obligation_type", "title", "description", "clause_reference", "due_date", "recurrence", "notice_period_days", "owner", "escalation_to", "proof_description", "evidence_json", "status", "category", "activation", "summary", "details_json", "penalties"];
  const fields = [];
  const params = [];
...
export function getAllObligations() {
  return query(
    `SELECT co.*, d.name as document_name, d.doc_type as document_type, d.client as document_client, d.status as document_status
     ...
     ORDER BY
       d.name ASC,
       CASE WHEN co.activation = 'dormant' THEN 1 ELSE 0 END ASC,
       co.category ASC,
       co.due_date ASC,
       co.created_at DESC`
  );
...
export function activateObligationsByContract(documentId, categories) {
  const placeholders = categories.map(() => "?").join(", ");
  run(
    `UPDATE contract_obligations SET activation = 'active', updated_at = CURRENT_TIMESTAMP
     WHERE document_id = ? AND category IN (${placeholders}) AND activation = 'dormant'`,
    [documentId, ...categories]
  );
  return query(
    `SELECT * FROM contract_obligations WHERE document_id = ? AND category IN (${placeholders}) AND activation = 'active'`,
    [documentId, ...categories]
  );
}

export function activateAllObligations(documentId) {
  run(
    `UPDATE contract_obligations SET activation = 'active', updated_at = CURRENT_TIMESTAMP
     WHERE document_id = ? AND activation = 'dormant'`,
    [documentId]
  );
  return query(
    `SELECT * FROM contract_obligations WHERE document_id = ? AND activation = 'active'`,
    [documentId]
  );
}
...
export function getContractSummary(documentId) {
  ...
  const obligations = query(
    `SELECT category, activation, status, due_date FROM contract_obligations WHERE document_id = ?`,
    [documentId]
  );
  ...
  for (const ob of obligations) {
    ...
    if (!counts[cat]) counts[cat] = { total: 0, active: 0, dormant: 0, met: 0, overdue: 0 };
    counts[cat].total++;
    if (ob.activation === "dormant") counts[cat].dormant++;
    else if (ob.status === "met") counts[cat].met++;
    else {
      counts[cat].active++;
      if (ob.due_date && ob.due_date < now) counts[cat].overdue++;
    }
    if (ob.due_date && ob.due_date >= now && ob.status === "active" && ob.activation === "active") {
      if (!nextDeadline || ob.due_date < nextDeadline) nextDeadline = ob.due_date;
    }
  }

  // Normalize contract status (legacy documents may have "draft" etc.)
  const isContract = doc.doc_type === "contract" || doc.doc_type === "agreement";
  const contractStatuses = ["unsigned", "signed", "active", "terminated"];
