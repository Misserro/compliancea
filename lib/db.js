...
export async function initDb() {
  ...
  `);
  try { db.run(`ALTER TABLE organizations ADD COLUMN storage_policy TEXT NOT NULL DEFAULT 'local'`); } catch (e) {}
  ...
  // ── Migration Jobs extensions (Plan 035 Task 3) ─────────────────────
  ...
  `);
  db.run(`CREATE INDEX IF NOT EXISTS idx_wizard_blueprints_org ON wizard_blueprints(org_id)`);
  ...
  // ── Case priority (Plan 045) ──────────────────────────────────────────
  try { db.run(`ALTER TABLE legal_cases ADD COLUMN priority TEXT NOT NULL DEFAULT 'normal'`); } catch (e) {}
  ...
  `);
  db.run(`CREATE INDEX IF NOT EXISTS idx_token_usage_user_org ON token_usage(user_id, org_id)`);
  try { db.run(`ALTER TABLE token_usage ADD COLUMN cache_read_tokens INTEGER NOT NULL DEFAULT 0`); } catch (e) {}
  try { db.run(`ALTER TABLE token_usage ADD COLUMN cache_write_tokens INTEGER NOT NULL DEFAULT 0`); } catch (e) {}
  ...
  try { db.run(`ALTER TABLE documents ADD COLUMN classification_metadata TEXT`); } catch (e) {}

  // Backfill: assign all existing unassigned cases to org owner (fallback: first admin)
  run(`UPDATE legal_cases SET assigned_to = (
...
export function getAllDocuments(orgId, source) {
  const unmatchedCase = `,
    CASE
  ...
  let sourceFilter = '';
  if (source === 'gdrive') {
    sourceFilter = ` AND d.source = 'gdrive'`;
  } else if (source === 'uploaded') {
    sourceFilter = ` AND (d.source IS NULL OR d.source != 'gdrive')`;
  }
...
export function deleteDocument(id) {
  // Delete obligations and their tasks first (PRAGMA foreign_keys is not enabled)
  deleteObligationsByDocumentId(id);
  // Delete chunks (sql.js doesn't support ON DELETE CASCADE reliably)
...
export function updateDocumentMetadata(id, metadata) {
  const allowedFields = [
    ...
    "commencement_date", "expiry_date", "contract_type", "is_historical", "name",
...
export function spawnDueObligations(documentId) {
  ...
  // Activate next payment if current upcoming payment is within 30 days
  const soon = new Date();
    ...
    return query(
      `SELECT co.*, d.name as document_name
       ...
       WHERE co.status = 'active'
         ...
         AND (d.sync_status IS NULL OR d.sync_status != 'deleted')
         AND d.org_id = ?
...
export function getContractsWithSummaries(orgId) {
  if (orgId !== undefined) {
    return query(
      `SELECT
        ...
        d.commencement_date, d.expiry_date, d.contract_type, d.is_historical,
      ...
      WHERE d.doc_type IN ('contract', 'agreement') AND d.source = 'gdrive' AND d.org_id = ?
        AND d.status != 'archived'
        AND (d.sync_status IS NULL OR d.sync_status != 'deleted')
      GROUP BY d.id
      ORDER BY d.name ASC`,
  ...
  return query(
    ...
    FROM documents d
    LEFT JOIN contract_obligations co ON d.id = co.document_id
    WHERE d.doc_type IN ('contract', 'agreement') AND d.source = 'gdrive'
      AND d.status != 'archived'
      AND (d.sync_status IS NULL OR d.sync_status != 'deleted')
    GROUP BY d.id
    ORDER BY d.name ASC`
...
export function finalizeObligation(id, { note, documentId }) {
  ...
  const finalized = getObligationById(id);

  // Cascade: activate next payment obligation if this was a payment
  if (finalized && finalized.category === 'payments') {
    activateNextPaymentObligation(finalized.document_id);
  }
 ...
 * @param {number} [options.orgId] - Organization ID (required for org isolation)
 * @param {number} [options.userId] - Current user ID; used for access control filtering
 * @param {string} [options.orgRole] - Current user's org role ('member'|'admin'|'owner'); when 'member', results are filtered to cases assigned to userId only
 * @returns {Object[]}
 */
export function getLegalCases({ search, status, caseType, priority, orgId, userId, orgRole, sortBy, limit, offset } = {}) {
  // Build WHERE clauses (shared between data query and count query)
  let where = ` WHERE 1=1`;
  const whereParams = [];
  ...
  if (orgRole === 'member' && userId !== undefined && userId !== null) {
    where += ` AND lc.assigned_to = ?`;
    whereParams.push(userId);
  }
  if (search) {
    where += ` AND lc.title LIKE ?`;
  ...
  if (status) {
    ...
    if (statuses.length === 1) {
      ...
      where += ` AND lc.status IN (${statuses.map(() => '?').join(',')})`;
      whereParams.push(...statuses);
    }
  }
  if (caseType) {
    where += ` AND lc.case_type = ?`;
    whereParams.push(caseType);
  }
  if (priority) {
    where += ` AND lc.priority = ?`;
    whereParams.push(priority);
  }

  // Count query (separate for sql.js compatibility — no window functions)
  const countSql = `SELECT COUNT(*) as total FROM legal_cases lc` + where;
  const countResult = get(countSql, [...whereParams]);
  ...
  // Dynamic ORDER BY based on sortBy param
  switch (sortBy) {
    case 'deadline':
      // NULLS LAST: cases without deadlines appear at the end
      sql += ` ORDER BY CASE WHEN (SELECT MIN(cd2.due_date) FROM case_deadlines cd2 WHERE cd2.case_id = lc.id AND cd2.status = 'pending') IS NULL THEN 1 ELSE 0 END, (SELECT MIN(cd3.due_date) FROM case_deadlines cd3 WHERE cd3.case_id = lc.id AND cd3.status = 'pending') ASC`;
      break;
    case 'title':
      sql += ` ORDER BY lc.title ASC`;
      break;
    case 'priority':
      sql += ` ORDER BY CASE lc.priority WHEN 'urgent' THEN 0 WHEN 'high' THEN 1 WHEN 'normal' THEN 2 WHEN 'low' THEN 3 ELSE 2 END ASC`;
      break;
    case 'created':
    default:
  ...
  const cases = query(sql, dataParams);
  return { cases, total };
}

/**
 * Get a single legal case by ID, scoped to an organization.
 * @param {number} id
 * @param {number} orgId
 * @returns {Object|null}
 */
export function getLegalCaseById(id, orgId) {
  return get(`
    SELECT lc.*,
      u.name as assigned_to_name,
      (SELECT MIN(cd.due_date)
...
export function createLegalCase({
  ...
  const result = run(
    `INSERT INTO legal_cases (
      ...
      tags, extension_data, org_id, assigned_to, priority
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      title,
      caseType,
...
/**
 * Get a single setting value for an org
 * @param {number} orgId
 * @param {string} key
 * @returns {string|null}
 */
...
export function createOrganization(name, slug) {
  const result = run(`INSERT INTO organizations (name, slug, storage_policy) VALUES (?, ?, 'platform_s3')`, [name, slug]);
...
/**
 * Get all feature flag rows for an organization.
 * @param {number} orgId
 * @returns {Array<{feature: string, enabled: number}>}
 */
export function getOrgFeatures(orgId) {
  return query(`SELECT feature, enabled FROM org_features WHERE org_id = ?`, [orgId]);
}

/**
 * Set a single feature flag for an organization (upsert).
 * @param {number} orgId
 * @param {string} feature
 * @param {boolean} enabled
 */
export function setOrgFeature(orgId, feature, enabled) {
  run(`INSERT INTO org_features (org_id, feature, enabled, updated_at)
       VALUES (?, ?, ?, CURRENT_TIMESTAMP)
       ON CONFLICT(org_id, feature) DO UPDATE SET enabled = excluded.enabled, updated_at = excluded.updated_at`,
    [orgId, feature, enabled ? 1 : 0]);
}

// ── Platform S3 & Storage Policy (Plan 034 Task 3) ────────────────────

/**
 * Get all platform settings as key-value rows.
 * @returns {Array<{key: string, value: string}>}
 */
export function getPlatformSettings() {
  return query(`SELECT key, value FROM platform_settings`);
}
...
export function setPlatformSetting(key, value) {
  run(`INSERT INTO platform_settings (key, value, updated_at)
       VALUES (?, ?, CURRENT_TIMESTAMP)
...
/**
 * Delete all platform settings (removes platform S3 config).
 */
export function deletePlatformSettings() {
  run(`DELETE FROM platform_settings`);
}

/**
 * Update the storage policy for an organization.
 * @param {number} orgId
 * @param {string} policy - 'local' | 'platform_s3' | 'own_s3'
 */
export function updateOrgStoragePolicy(orgId, policy) {
  run(`UPDATE organizations SET storage_policy = ? WHERE id = ?`, [policy, orgId]);
}

/**
 * Get the storage policy for an organization.
 * @param {number} orgId
 * @returns {{storage_policy: string}|undefined}
 */
export function getOrgStoragePolicy(orgId) {
  return get(`SELECT storage_policy FROM organizations WHERE id = ?`, [orgId]);
}
...
export function createMigrationJob() {
  const result = run(`INSERT INTO migration_jobs (status) VALUES ('pending')`);
  return result.lastInsertRowId;
}

/**
 * Update a migration job with a partial patch.
 * @param {number} id
 * @param {Object} patch - fields to update (camelCase keys mapped to snake_case columns)
 */
export function updateMigrationJob(id, patch) {
  const keyMap = {
    status: "status",
  ...
  if (fields.length === 0) return;
  params.push(id);
  run(`UPDATE migration_jobs SET ${fields.join(", ")} WHERE id = ?`, params);
}

/**
 * Get the most recent migration job.
 * @returns {Object|undefined}
 */
export function getLatestMigrationJob() {
  return get(`SELECT * FROM migration_jobs ORDER BY created_at DESC LIMIT 1`);
}

/**
 * Create a migration job scoped to a specific org.
 * @param {number} orgId
 * @param {string} migrationType - 'local_to_platform_s3' or 'own_s3_to_platform_s3'
 * @returns {number} Job ID
 */
...
/**
 * Get the most recent migration job for a specific org.
 * @param {number} orgId
 * @returns {Object|undefined}
 */
...
/**
 * Create a custom wizard blueprint for an organization.
 * @param {number} orgId
 * @param {string} name
 * @param {string} sectionsJson - JSON string of sections array
 * @returns {number} New blueprint ID
 */
export function createWizardBlueprint(orgId, name, sectionsJson) {
  const result = run(
    `INSERT INTO wizard_blueprints (org_id, name, sections_json) VALUES (?, ?, ?)`,
    [orgId, name, sectionsJson || '[]']
  );
  return result.lastInsertRowId;
}

/**
 * List all wizard blueprints for an organization.
 * @param {number} orgId
 * @returns {Object[]}
 */
export function getWizardBlueprints(orgId) {
  return query(
    `SELECT * FROM wizard_blueprints WHERE org_id = ? ORDER BY name ASC`,
    [orgId]
  );
}

/**
 * Get a single wizard blueprint by ID, scoped to an organization.
 * @param {number} id
 * @param {number} orgId
 * @returns {Object|undefined}
 */
export function getWizardBlueprintById(id, orgId) {
  return get(
    `SELECT * FROM wizard_blueprints WHERE id = ? AND org_id = ?`,
    [id, orgId]
  );
}

/**
 * Update a wizard blueprint (name and/or sections_json).
 * @param {number} id
 * @param {number} orgId
 * @param {Object} data - { name?, sections_json? }
 */
export function updateWizardBlueprint(id, orgId, data) {
  const setClauses = [];
  const params = [];
  ...
  if (data.sections_json !== undefined) {
    ...
    params.push(data.sections_json);
  }
  ...
  run(
    `UPDATE wizard_blueprints SET ${setClauses.join(', ')} WHERE id = ? AND org_id = ?`,
    params
  );
}

/**
 * Delete a wizard blueprint, scoped to an organization.
 * @param {number} id
 * @param {number} orgId
 */
export function deleteWizardBlueprint(id, orgId) {
  run(`DELETE FROM wizard_blueprints WHERE id = ? AND org_id = ?`, [id, orgId]);
}
...
/**
 * Get firm statistics for the admin "My law firm" tab.
 * Returns case counts grouped by status, count of cases finalized in last
 * 30 days, and member roster with profile fields + assigned case count.
 * @param {number} orgId
 * @returns {{ statsByStatus: Object[], finalizedLast30Days: number, members: Object[] }}
 */
export function getFirmStats(orgId) {
  const statsByStatus = query(
  ...
  const finalizedLast30Days = row ? row.count : 0;

  const members = query(
    `SELECT om.user_id, u.name, u.email, om.role,
            om.first_name, om.last_name, om.phone,
            om.specialization, om.bar_registration_number,
            COALESCE(cc.cnt, 0) as assigned_case_count
     FROM org_members om
     JOIN users u ON u.id = om.user_id
     LEFT JOIN (
     ...
     ) cc ON cc.assigned_to = om.user_id
     WHERE om.org_id = ?
     ORDER BY om.joined_at ASC`,
    [orgId, orgId]
  );

  return { statsByStatus, finalizedLast30Days, members };
}

/**
 * Update a member's lawyer profile fields on org_members.
 * Validates the target user belongs to the org before updating.
 * @param {number} orgId
 * @param {number} userId
 * @param {Object} fields - allowed keys: first_name, last_name, phone, specialization, bar_registration_number
 * @returns {Object|null} The updated member record, or null if user not in org
 */
export function updateMemberProfile(orgId, userId, fields) {
  const member = get(
    `SELECT om.user_id FROM org_members om WHERE om.org_id = ? AND om.user_id = ?`,
    [orgId, userId]
  );
  if (!member) return null;

  const allowedFields = [
    "first_name", "last_name", "phone", "specialization", "bar_registration_number",
  ];
  ...
  run(
    `UPDATE org_members SET ${setClauses.join(', ')} WHERE org_id = ? AND user_id = ?`,
    params
  );
...
/**
 * Dashboard data for the Legal Hub section.
 * Returns open-case stats by status, upcoming deadlines (30 days), and 5 most recent cases.
 * When orgRole is 'member', all queries are scoped to cases assigned to userId.
 * @param {number} orgId
 * @param {number} userId
 * @param {string} orgRole
 * @returns {{ statsByStatus: Array, upcomingDeadlines: Array, recentCases: Array }}
 */
export function getLegalHubDashboardData(orgId, userId, orgRole) {
  const memberFilter = orgRole === 'member' ? ' AND lc.assigned_to = ?' : '';
  const baseParams = orgRole === 'member' ? [orgId, userId] : [orgId];

  // Open cases (non-closed, non-archived) grouped by status
  const statsByStatus = query(
    `SELECT lc.status, COUNT(*) as count
     FROM legal_cases lc
     WHERE lc.org_id = ?
       AND lc.status NOT IN ('closed', 'archived')${memberFilter}
     GROUP BY lc.status
     ORDER BY lc.status ASC`,
    baseParams
  );
  ...
  const upcomingDeadlines = query(
    `SELECT cd.id, cd.case_id, lc.title AS case_title, cd.title, cd.deadline_type, cd.due_date
     FROM case_deadlines cd
     JOIN legal_cases lc ON lc.id = cd.case_id
     WHERE lc.org_id = ?
       AND cd.status = 'pending'
       AND cd.due_date >= date('now')
       AND cd.due_date <= date('now', '+30 days')${memberFilter}
  ...
  const recentCases = query(
    `SELECT lc.id, lc.title, lc.status, lc.case_type, lc.created_at,
            u.name AS assigned_to_name
     FROM legal_cases lc
     LEFT JOIN users u ON lc.assigned_to = u.id
     WHERE lc.org_id = ?${memberFilter}
...
/**
 * Get upcoming (within N days) and overdue pending deadlines across all
 * user-visible cases. Respects org + role visibility (members see only
 * their assigned cases).
 * @param {Object} opts
 * @param {number} opts.orgId
 * @param {number} opts.userId
 * @param {string} opts.orgRole
 * @param {number} [opts.days=7]
 * @returns {Object[]}
 */
export function getUpcomingDeadlinesForUser(orgId, userId, orgRole, days) {
  ...
  let sql = `
    ...
    WHERE cd.status = 'pending'
      AND cd.due_date <= date('now', '+' || ? || ' days')
      AND lc.org_id = ?
  `;
  const params = [days, orgId];

  if (orgRole === 'member' && userId !== undefined && userId !== null) {
    sql += ` AND lc.assigned_to = ?`;
    params.push(userId);
  }
...
/**
 * Log AI token usage for a single request (fire-and-forget).
 * Errors are swallowed silently — a failed write must never affect the AI response.
 * @param {Object} params
 * @param {number} params.userId
 * @param {number} params.orgId
 * @param {string} params.route - e.g. '/api/ask'
 * @param {string} params.model - e.g. 'sonnet', 'haiku', 'voyage'
 * @param {number} params.inputTokens
 * @param {number} params.outputTokens
 * @param {number} [params.voyageTokens=0]
 * @param {number} [params.costUsd=0]
 */
export function logTokenUsage({ userId, orgId, route, model, inputTokens, outputTokens, voyageTokens = 0, costUsd = 0, cacheReadTokens = 0, cacheWriteTokens = 0 }) {
  try {
    run(
      `INSERT INTO token_usage (user_id, org_id, route, model, input_tokens, output_tokens, voyage_tokens, cost_usd, cache_read_tokens, cache_write_tokens)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [userId, orgId, route, model, inputTokens, outputTokens, voyageTokens, costUsd, cacheReadTokens, cacheWriteTokens]
    );
  } catch (e) {
    // Silent — token logging must never break the caller
  }
}

/**
 * Get aggregated token usage summary per user/org.
 * @param {Object} [filters={}]
 * @param {number} [filters.orgId] - Optional org filter
 * @returns {Object[]} Array of { userId, userName, userEmail, orgId, orgName, claudeInputTokens, claudeOutputTokens, voyageTokens, estimatedCostUsd }
 */
export function getTokenUsageSummary(filters = {}) {
  let sql = `
    SELECT
      ...
      tu.org_id,
      o.name AS org_name,
      SUM(tu.input_tokens) AS claude_input_tokens,
      SUM(tu.output_tokens) AS claude_output_tokens,
      SUM(tu.voyage_tokens) AS voyage_tokens,
      SUM(tu.cost_usd) AS estimated_cost_usd
    FROM token_usage tu
    JOIN users u ON tu.user_id = u.id
    JOIN organizations o ON tu.org_id = o.id
  `;
  const params = [];
  ...
  sql += ` GROUP BY tu.user_id, tu.org_id ORDER BY estimated_cost_usd DESC`;
  ...
  return rows.map(row => ({
    ...
    voyageTokens: row.voyage_tokens,
...
export function findMatchingContract(orgId, { parties = [], contractRef = null, vendorName = null } = {}) {
  const contracts = query(
    `SELECT id, name, contracting_company, contracting_vendor
     FROM documents
     WHERE org_id = ? AND doc_type IN ('contract', 'agreement')
       AND (status IS NULL OR status != 'terminated')`,
    [orgId]
  );

  const searchTerms = [
    ...(parties || []).map(p => p.toLowerCase()),
    vendorName ? vendorName.toLowerCase() : null,
  ].filter(Boolean);
  const refLower = contractRef ? contractRef.toLowerCase() : null;

  const scored = contracts.map(c => {
    let score = 0;
    const fields = [
      (c.contracting_company || "").toLowerCase(),
      (c.contracting_vendor || "").toLowerCase(),
      (c.name || "").toLowerCase(),
    ];

    for (const term of searchTerms) {
      if (fields.some(f => f.includes(term) || term.includes(f))) score += 1;
    }
    if (refLower && fields.some(f => f.includes(refLower))) {
      score += 1;
    }

    return { contractId: c.id, score };
  });

  const best = scored.filter(s => s.score > 0).sort((a, b) => b.score - a.score);
  if (best.length === 0) return null;
  if (best.length > 1 && best[0].score === best[1].score) return null;
  if (best[0].score < 0.5) return null;
  return { contractId: best[0].contractId, confidence: best[0].score };
}

export function insertContractDocument(contractId, documentId, documentType, label) {
  const doc = get(`SELECT name FROM documents WHERE id = ?`, [documentId]);
  const result = run(
    `INSERT INTO contract_documents (contract_id, document_id, file_name, document_type, label)
     VALUES (?, ?, ?, ?, ?)`,
    [contractId, documentId, doc ? doc.name : null, documentType || "annex", label || null]
  );
  return result.lastInsertRowId;
}

export function getUnmatchedAnnexes(orgId) {
  return query(
    `SELECT d.id, d.name, d.path, d.full_text, d.classification_metadata
     FROM documents d
     WHERE d.doc_type = 'annex' AND d.org_id = ?
       AND NOT EXISTS (SELECT 1 FROM contract_documents cd WHERE cd.document_id = d.id)`,
    [orgId]
  );
}

export function getUnmatchedInvoices(orgId) {
  return query(
    `SELECT d.id, d.name, d.path, d.full_text, d.classification_metadata
     FROM documents d
     WHERE d.doc_type = 'invoice' AND d.org_id = ?
       AND NOT EXISTS (SELECT 1 FROM contract_invoices ci WHERE ci.document_id = d.id)`,
    [orgId]
  );
}

export function insertContractInvoiceFromGDrive(contractId, documentId, invoiceData) {
  const doc = get(`SELECT path FROM documents WHERE id = ?`, [documentId]);
  const result = run(
    `INSERT INTO contract_invoices
       (contract_id, document_id, amount, currency, description, date_of_issue, date_of_payment, invoice_file_path)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      contractId,
      documentId,
...
export function getDocumentFullText(docId) {
  const row = get(`SELECT full_text FROM documents WHERE id = ?`, [docId]);
