 ...
 * @param {number|null} [options.userId] - User who performed the action
 * @param {number|null} [options.orgId] - Organization context
 */
export function logAction(entityType, entityId, action, details = null, options = {}) {
  ...
  run(
    `INSERT INTO audit_log (entity_type, entity_id, action, details, user_id, org_id)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [entityType, entityId, action, detailsStr, userId, orgId]
  );
}
 ...
 * @param {number} [filters.orgId] - Filter by organization ID
...
export function getAuditLog(filters = {}) {
  ...
  return query(
    `SELECT id, entity_type, entity_id, action, details, user_id, org_id, created_at
     FROM audit_log
     ${where}
