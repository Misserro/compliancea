...
import { query, run, get, getOrgSetting, updateDocumentMetadata, updateDocumentProcessed, insertObligation, createTaskForObligation, findMatchingContract, insertContractDocument, insertContractInvoiceFromGDrive, getUnmatchedAnnexes, getUnmatchedInvoices } from "./db.js";
import { logAction } from "./audit.js";
import { shouldSync, scanGDrive, getGDriveStatus } from "./gdrive.js";
import { extractContractTerms, classifyGDriveDocument } from "./contracts.js";
      ...
      if (orgsWithGDrive.length === 0) {
        ...
        for (const { org_id } of orgsWithGDrive) {
          try {
            ...
            // Re-match previously unmatched annexes and invoices for this org
            try {
              const rematchResult = await rematchUnlinkedDocuments(org_id);
              results.gdrive[org_id].rematch = rematchResult;
            } catch (err) {
              results.gdrive[org_id].rematchError = err.message;
            }
          } catch (err) {
...
/**
 * Auto-process a GDrive document.
 * Classifies the document (contract / annex / invoice / other) and branches accordingly.
 * Contracts follow the existing extraction + obligations path.
 * Annexes and invoices are matched to parent contracts when possible.
 * @param {number} docId
...
async function processGDriveDocument(docId, localPath, orgId) {
  ...
  const classifyResult = await classifyGDriveDocument(text);
  const { classification, annexParentReference, invoiceData } = classifyResult;

  // Mark as processed (common to all branches)
  updateDocumentProcessed(docId, wordCount);

  if (classification === "contract") {
    // ── Contract path (unchanged from Plan 053) ─────────────────────
  ...
  } else if (classification === "annex") {
    // ── Annex path ──────────────────────────────────────────────────
    const metaUpdate = {
      doc_type: "annex",
      full_text: text,
    };
    if (annexParentReference) {
      metaUpdate.classification_metadata = JSON.stringify(annexParentReference);
    }
    updateDocumentMetadata(docId, metaUpdate);

    // Attempt to match to a parent contract
    let matched = false;
    if (annexParentReference) {
      const match = findMatchingContract(orgId, {
        parties: annexParentReference.parties || [],
        contractRef: annexParentReference.contractNumber || null,
        vendorName: null,
      });
      if (match) {
        insertContractDocument(match.contractId, docId, "annex", annexParentReference.contractTitle || null);
        matched = true;
      }
    }
    ...
    if (invoiceData) {
      metaUpdate.classification_metadata = JSON.stringify(invoiceData);
    }
    updateDocumentMetadata(docId, metaUpdate);

    // Attempt to match to a contract via vendor name
    let matched = false;
    if (invoiceData) {
      const match = findMatchingContract(orgId, {
        parties: invoiceData.vendorName ? [invoiceData.vendorName] : [],
        contractRef: invoiceData.contractReference || null,
        vendorName: invoiceData.vendorName || null,
      });
      if (match) {
        insertContractInvoiceFromGDrive(match.contractId, docId, invoiceData);
        matched = true;
      }
    }
...
/**
 * Re-attempt matching for unmatched annexes and invoices.
 * Uses stored classification_metadata (no new Claude calls).
 * @param {number} orgId
 * @returns {{ annexesMatched: number, invoicesMatched: number }}
 */
async function rematchUnlinkedDocuments(orgId) {
  let annexesMatched = 0;
  let invoicesMatched = 0;

  // Re-match unmatched annexes
  const unmatchedAnnexes = getUnmatchedAnnexes(orgId);
  for (const doc of unmatchedAnnexes) {
    try {
      let parentRef = null;
      if (doc.classification_metadata) {
        try { parentRef = JSON.parse(doc.classification_metadata); } catch (e) { /* invalid JSON, skip */ }
      }
      if (!parentRef) continue;

      const match = findMatchingContract(orgId, {
        parties: parentRef.parties || [],
        contractRef: parentRef.contractNumber || null,
        vendorName: null,
      });
      if (match) {
        insertContractDocument(match.contractId, doc.id, "annex", parentRef.contractTitle || null);
        logAction("document", doc.id, "gdrive_annex_rematched", { contractId: match.contractId });
        annexesMatched++;
      }
    } catch (err) {
      console.error(`rematch annex ${doc.id}:`, err.message);
    }
  }

  // Re-match unmatched invoices
  const unmatchedInvoices = getUnmatchedInvoices(orgId);
  for (const doc of unmatchedInvoices) {
    try {
      let invData = null;
      if (doc.classification_metadata) {
        try { invData = JSON.parse(doc.classification_metadata); } catch (e) { /* invalid JSON, skip */ }
      }
      if (!invData) continue;

      const match = findMatchingContract(orgId, {
        parties: invData.vendorName ? [invData.vendorName] : [],
        contractRef: invData.contractReference || null,
        vendorName: invData.vendorName || null,
      });
      if (match) {
        insertContractInvoiceFromGDrive(match.contractId, doc.id, invData);
        logAction("document", doc.id, "gdrive_invoice_rematched", { contractId: match.contractId });
        invoicesMatched++;
      }
    } catch (err) {
      console.error(`rematch invoice ${doc.id}:`, err.message);
    }
  }

  return { annexesMatched, invoicesMatched };
}
